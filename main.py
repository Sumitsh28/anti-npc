...
from analyzer import analyze_issue_and_repo, analyze_user
...
def github_webhook():
    ...
    if event == 'issue_comment' and data.get('action') == 'created':
        ...
        comment_body_lower = comment_body_original.lower()
        
        trigger_phrases = ['please assign me', 'i want to work on this', 'assign this to me', 'can i take this?']
        
        if data.get('comment', {}).get('user', {}).get('type') == 'Bot':
            return "Ignoring bot comment", 200
            
        if any(phrase in comment_body_lower for phrase in trigger_phrases):
            ...
            try:
                ...
                print("Authenticating and fetching GitHub data...")
                ...
                if not all([issue_data, repo_data, user_data]):
                    print("Failed to fetch all required data from GitHub.")
